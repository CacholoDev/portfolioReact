const educationItems = [
    {
        ...
        place: "IES San Clemente - Santiago de Compostela",
        period: "2023 - 2026",
        highlights: ["Full Stack", "Arquitectura web", "Bases de datos", "Despliegue", "API-Rest", "Control de versiones","Contenedores", "Scrum"],
    },
    {
    ...
    return (
        <section id="education" className="space-y-5">
            <div className="flex items-end justify-between">
                ...
                <p className="text-xs uppercase tracking-[0.2em] text-cyan-300/80">Learning Path</p>
            </div>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                {educationItems.map((item) => (
                    <article
                        key={item.title}
                        className="group relative min-h-52 overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/40 p-5 transition-all duration-300 hover:-translate-y-1 hover:border-cyan-300/60 hover:shadow-lg hover:shadow-cyan-400/15"
                    >
                        ...
                        <div className="relative z-10 flex h-full gap-4">
                            <div className="flex flex-col items-center">
                                <span className="mt-1 h-3 w-3 rounded-full bg-cyan-300 shadow-[0_0_16px_rgba(103,232,249,.6)]" />
                            ...
                            <div className="flex flex-1 flex-col">
                                ...
                                <p className="text-sm font-medium text-slate-200">{item.subtitle}</p>
                                ...
                                <ul className="mt-4 flex flex-wrap gap-2">
                                    {item.highlights.map((point) => (
                                        <li
                                            key={point}
                                            className="rounded-md border border-slate-700/80 bg-slate-800/60 px-2 py-1 text-xs text-slate-200 transition-colors duration-300 group-hover:border-slate-600"
                                        >
                                            {point}
                                        </li>
                                    ))}
